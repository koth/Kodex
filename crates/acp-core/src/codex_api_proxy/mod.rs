...
const SHELL_TOOL_INSTRUCTIONS: &str = "Shell command compatibility rule: Run project commands from the directory that owns the relevant manifest or toolchain file, such as package.json or Cargo.toml. Prefer local project executables and scripts over bare npx or package-manager commands that may download from the network. When piping output through head, tail, grep, or similar filters, preserve the original failing exit status with set -o pipefail or avoid the pipe. Do not repeatedly retry the exact same failing shell command; inspect the error and change strategy first. File editing rule: never use shell/Python/Node to create or edit source files (`Path.write_text`, `open(...,'w')`, `fs.writeFile`, `>`, `>>`, `tee`, `sed -i`). Use Edit/MultiEdit/Write or apply_patch instead.";
const NON_GPT_EDIT_BRIDGE_INSTRUCTIONS: &str = "Editing tool compatibility rule: Prefer the Edit, MultiEdit, and Write tools for file changes when they are available. Kodex converts those Claude-style editing tools into apply_patch internally, so using them satisfies the apply_patch requirement. Use raw apply_patch only as a fallback when the change cannot be represented with Edit, MultiEdit, or Write. Never edit files via shell/Python/Node scripts, heredocs, redirection, or in-place sed/perl — those bypass reviewable patches. Use shell commands only for inspection or validation.";
